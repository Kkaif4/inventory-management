...
export function CurrentStockClient({ stocks }: { stocks: any[] }) {
  ...
  const columns: ColumnDef<any>[] = [
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className="text-2xs font-medium text-text-muted">
              {t("unit")}: {row.original.variant.product.baseUnit}
            </span>
          </div>
        </div>
      ),
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const type = row.original.warehouse ? t("warehouse") : t("outlet");
        return (
    ...
    },
    {
      ...
      header: () => <div className="text-right">{t("physicalQty")}</div>,
      size: 120,
      cell: ({ getValue, row }) => (
    ...
    },
    {
      ...
      header: () => <div className="text-right">{t("inTransit")}</div>,
      size: 100,
      cell: ({ getValue }) => {
  ...
  const breadcrumbs = [
    { label: common("groups.inventory") },
    { label: t("title"), href: "/dashboard/inventory/current-stock" },
  ];
