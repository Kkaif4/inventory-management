import { getVendorLedger } from "@/actions/purchase/vendors";
...
export default async function VendorLedgerPage({
  ...
  const result = await getVendorLedger(id, from as string, to as string);
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          {
            label: party.name,
            href: `/dashboard/purchase/vendors/${id}`,
          },
          { label: "Ledger" },
        ]}
      />
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border space-y-4">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">Payables Statement</p>
          </div>
            ...
            <TableBody>
              ...
              {entries.map((entry: any) => (
                <TableRow key={entry.id}>
                  ...
                  {/* For vendors, Debit represents payments lowering the liability */}
                  <TableCell className="text-right text-emerald-700">
                    {entry.debit > 0 ? formatCurrency(entry.debit) : ""}
                  </TableCell>
                  {/* Credit represents bills increasing the liability */}
