...
import { recordInvoicePayment, recordInvoiceMultiplePayments, getOutletAccounts } from "@/actions/sales/payment";
  ...
  const onSubmit = async (data: RecordPaymentFormValues) => {
    if (isSplit) {
      ...
      try {
        ...
        const res = await recordInvoiceMultiplePayments({
          invoiceId: invoice.id,
          outletId: invoice.outletId,
          partyId: invoice.partyId,
        ...
        if (res.success && res.data) {
          const { invoiceStatus, remaining } = res.data;
          toast.success(
            invoiceStatus === "PAID"
              ? `Payments totalling ₹${totalEntered.toFixed(2)} recorded. Invoice ${invoice.txnNumber} is now Paid.`
          ...
          <div className="flex items-center justify-between p-4 border border-slate-100 rounded-xl bg-slate-50/50">
            ...
            <button
              ...
              className={cn(
                ...
                isSplit ? "bg-emerald-600" : "bg-slate-200"
              )}
            >
              <span
                className={cn(
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="h-8 text-[10px] font-bold border-slate-200 text-slate-700 bg-white hover:bg-slate-50 rounded-lg shadow-sm"
                  onClick={() =>
                    setSplitPayments([
                      ...splitPayments,
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="w-[110px] shrink-0">
                          <select
                            ...
                            className="w-full h-9 text-xs border border-slate-200 rounded-lg px-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none bg-slate-50 font-medium transition-all"
                          >
