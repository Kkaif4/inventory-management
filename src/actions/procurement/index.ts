...
// ─── Get purchase orders with server-side pagination ──────────────────────
export async function getPurchaseOrdersPaginated(
  ...
  return withErrorHandler(async (): Promise<PaginatedResult<any>> => {
    ...
    const andClauses: any[] = [{ type: "PURCHASE_ORDER" }, { outletId }];
...
export async function getPurchaseOrderById(id: string, outletId: string) {
  return withErrorHandler(async () => {
    ...
    if (po && po.outletId !== outletId) {
      throw new ForbiddenError("Purchase order not found in this outlet");
    }
  ...
  return withErrorHandler(async () => {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const totalCgst = roundToTwo(
        source.items.reduce((a, b) => a + b.cgst, 0),
      );
      const totalSgst = roundToTwo(
        source.items.reduce((a, b) => a + b.sgst, 0),
      );
...
// ─── Get purchase bills with server-side pagination ────────────────────────
export async function getBillsPaginated(
  ...
  return withErrorHandler(async (): Promise<PaginatedResult<any>> => {
    ...
    const andClauses: any[] = [{ type: "PURCHASE_BILL" }, { outletId }];
