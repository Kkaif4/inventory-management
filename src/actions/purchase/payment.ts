...
import type { RecordPaymentFormValues, RecordMultiplePaymentsFormValues } from "@/validations/payment.validation";
...
// ─── Record multiple payments against a posted vendor bill ─────────────────
export async function recordVendorBillMultiplePayments(
  ...
  return withErrorHandler(async () => {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      // 2. Compute outstanding
      const alreadyPaid = await tx.payment.aggregate({
        where: { invoiceId: data.invoiceId },
        _sum: { amount: true },
      });
      const totalPaidPrior = roundToTwo(alreadyPaid._sum.amount ?? 0);
      const outstandingPrior = roundToTwo(bill.grandTotal - totalPaidPrior);
      ...
      if (totalNewPaymentAmount > outstandingPrior + 0.005) {
        throw new ValidationError(
          `Payment amount ₹${totalNewPaymentAmount} exceeds outstanding balance of ₹${outstandingPrior}.`,
        );
      }
      ...
      for (const p of data.payments) {
        ...
        if (!p.bankAccountId) {
          throw new ValidationError("Payment account is required for all payment rows");
        }
      ...
      const isFullyPaid = totalPaidNew >= bill.grandTotal - 0.005;
      ...
      if (bill.partyId) {
        ...
        let paymentAmount = totalNewPaymentAmount;
        if (totalNewPaymentAmount > totalOutstanding + 0.005) {
          const creditAmount = roundToTwo(totalNewPaymentAmount - totalOutstanding);
          await tx.party.update({
            where: { id: bill.partyId },
            data: {
              creditBalance: {
      ...
      return {
        billStatus: isFullyPaid ? "PAID" : "PARTIALLY_PAID",
        remaining: roundToTwo(bill.grandTotal - totalPaidNew),
      };
    });
  });
}
